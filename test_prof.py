...
print("Testing /professores GET")
r = req_with(f"{XANO_API_URL}/professores", "GET", token=t1)
...
print("\nTesting /disciplinas GET")
r = req_with(f"{XANO_API_URL}/disciplinas", "GET", token=t1)
print("Status:", r.status_code)
try:
    print("Data:", json.dumps(r.json()[:2], indent=2))
except Exception:
    print("Text:", r.text)

print("\nTesting /professores POST")
r_post = req_with(f"{XANO_API_URL}/professores", "POST", {"nome": "Prof Teste 1"}, token=t1)
